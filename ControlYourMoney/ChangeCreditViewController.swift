...
class ChangeCreditViewController: UIViewController, UITextFieldDelegate, UITextViewDelegate{
    ...
    func setupCreditLable(){
        ...
        save.addTarget(self,action:"saveCredit",forControlEvents:.TouchUpInside)
        ...
        alreadyPay.addTarget(self,action:"alreadyPay",forControlEvents:.TouchUpInside)
        self.view.addSubview(alreadyPay)
        
        let alreadyPayAll = UIButton(frame: CGRectMake(20, 500, self.view.frame.size.width - 40, 44))
        ...
        alreadyPayAll.addTarget(self,action:"alreadyPayAll",forControlEvents:.TouchUpInside)
        self.view.addSubview(alreadyPayAll)
        
        let deleteBtn = UIButton(frame: CGRectMake(20, 560, self.view.frame.size.width - 40, 44))
    ...
    func alreadyPayAll(){
        ...
        InsertTotaleData(getCanUseToFloat() - Float(numberCreditData.text!)! * Float(periodsCreditData.text!)!, time: getTime())
        
        DeleteCreditData(changeIndex)
        showToast().showToast("还款成功！")
                ...
                if(stringIsFloat(numberCreditData.text! as String)){
                    if(stringIsInt(dateCreditData.text! as String)){
                        if(Int(dateCreditData.text!) < 29 && Int(dateCreditData.text!) > 0){
                            ...
                            InsertTotaleData(getCanUseToFloat() - Float(numberCreditData.text!)!, time: getTime())
